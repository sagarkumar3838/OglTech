-- ============================================
-- ULTIMATE FIX FOR MULTIPLE ANSWERS SUPPORT
-- ============================================
-- This script handles ALL edge cases and fixes everything
...
BEGIN
    ...
    SELECT COUNT(*) INTO invalid_count FROM practice_questions 
    WHERE question_type NOT IN ('single', 'multiple') OR question_type IS NULL;
    ...
    RAISE NOTICE '  - Invalid values: %', invalid_count;
    RAISE NOTICE '';
    
    IF invalid_count > 0 THEN
        RAISE NOTICE '⚠️  WARNING: % questions have invalid question_type!', invalid_count;
        RAISE NOTICE '   Run this query to see them:';
        RAISE NOTICE '   SELECT * FROM practice_questions WHERE question_type NOT IN (''single'', ''multiple'');';
    ELSE
        RAISE NOTICE '✅ All question_type values are valid!';
    END IF;
